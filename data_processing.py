...
def torax(test_size=0.2):
    ...
    x_train, x_test, y_train, y_test = sklearn.model_selection.train_test_split(data.iloc[:, :16], data.iloc[:, 16],
...
def bike(test_size=0.2):
    ...
    x_train, x_test, y_train, y_test = sklearn.model_selection.train_test_split(data.iloc[:, 1:16], data.iloc[:, 16],
    ...
    #Normalize for better results in NN
...
def student(test_size=0.2):
    ...
    x_train, x_test, y_train, y_test = sklearn.model_selection.train_test_split(data.iloc[:, :30], data.iloc[:, 32],
                                                                                random_state=0, test_size=test_size)
